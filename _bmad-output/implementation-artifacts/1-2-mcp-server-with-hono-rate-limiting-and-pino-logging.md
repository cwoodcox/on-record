...
- [x] [AI-Review][LOW] Pin `@hono/node-server` to exact version — `^1.x` is unpinned; architecture mandates pinning packages that underpin the MCP transport pattern (`c.env.incoming/outgoing`). [apps/mcp-server/package.json:17]
- [x] [AI-Review][LOW] Normalize semver range format — `^1.x`, `^3.x`, `^13.x` should be `^1.0.0`, `^3.0.0`, `^13.0.0`. [apps/mcp-server/package.json]
...
- [x] [AI-Review][LOW] Add timeout to `drainResponse` — if neither `finish` nor `close` fires (broken connection / framework edge case), the handler coroutine leaks forever; add `setTimeout(resolve, 30_000)` fallback. [apps/mcp-server/src/index.ts:37-43]
- [x] [AI-Review][LOW] Add `process.on('unhandledRejection')` handler — unhandled rejections crash Node.js 15+; add a handler that logs at `error` level with `source: 'app'` before exit. [apps/mcp-server/src/index.ts]
...
- `src/env.ts`: zod schema validating PORT, NODE_ENV, UTAH_LEGISLATURE_API_KEY, UGRC_API_KEY. Uses `process.exit(1)` on validation failure with `console.error` (pre-pino). Exports `validateEnv()` and `getEnv()`.
...
- ✅ Resolved [HIGH]: ESLint migrated to flat config (`eslint.config.js`); `.eslintrc.json` deleted; `no-console` rule verified enforcing with exit code 1 on violation. AC6 now enforced.
- ✅ Resolved [HIGH-partial]: `tsc --noEmit` passes (0 errors), `eslint src/` passes (0 violations). Runtime verification (server startup, curl, rate-limit breach) remains pending manual execution with live environment vars.
- ✅ Resolved [MEDIUM]: Rate-limit `keyGenerator` comment expanded to document Railway proxy trust model and MVP-accepted risk.
- ✅ Resolved [MEDIUM]: Removed dead `export { _env as env }` from `env.ts`.
...
- `apps/mcp-server/src/env.test.ts` (new)
...
- `apps/mcp-server/eslint.config.js` (new — replaces .eslintrc.json)
- `apps/mcp-server/.eslintrc.json` (deleted)
...
- 2026-03-02: Resolved remaining 4 LOW items. (1) Pinned `@hono/node-server` to exact version `1.19.9`; (2) semver ranges already normalized to `^N.0.0` form; (3) added 30s `setTimeout` fallback to `drainResponse` to prevent leaked coroutines; (4) added `process.on('unhandledRejection')` handler logging at `error` level with `source: 'app'` before `process.exit(1)`. All review items now resolved.
