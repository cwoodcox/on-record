...
- pnpm `approve-builds` required for esbuild/sharp/unrs-resolver — resolved via `pnpm.onlyBuiltDependencies` in root `package.json` and `.npmrc`
...
- `.gitignore` (created)
- `.npmrc` (created — approve-builds for esbuild/sharp/unrs-resolver)
...
- `apps/web/package.json` (modified — name, workspace deps, shadcn deps)
- `apps/web/tsconfig.json` (modified — extends @on-record/typescript-config/nextjs.json)
- `apps/web/.env.example` (created)
- `apps/web/components.json` (created by shadcn/ui init)
- `apps/web/src/app/globals.css` (modified by shadcn/ui init — CSS variables added)
