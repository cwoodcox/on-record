...
PR 31 review (2026-04-26). Three reviewers: Blind Hunter (adversarial, no context), Edge Case Hunter (boundary analysis with project read), Acceptance Auditor (spec/AC + conventions). 12 patches, 1 decision-needed (multi-part), 6 deferred, 5 dismissed as noise.

**Decision needed (resolve before patches):**

- [ ] [Review][Decision] **Scope creep — three unmentioned change sets in this PR** — The diff modifies 23 files but the spec File List only declares 10. Three substantial change sets are *not* in story 4.10's spec, ACs, completion notes, or File List:
  (a) **pino logger refactor** — `apps/mcp-server/src/lib/logger.ts` rewritten from a synchronous console-delegate to a pino-shaped Proxy with `getLogger()` factory and `cfWrite()` level-routing; `pino: 10.3.1` added to `apps/mcp-server/package.json`; `apps/mcp-server/eslint.config.cjs` adds `'no-console': ['error', { allow: ['error'] }]`; `CLAUDE.md` rewrites the logging architectural rule (was: "call console.log/warn/error directly … No logger abstraction, no pino"; now: "console.log FORBIDDEN … only console.error"); `apps/mcp-server/src/lib/logger.test.ts` rewritten; mock-comment edits in `apps/mcp-server/src/tools/legislator-lookup.test.ts` and `apps/mcp-server/src/providers/utah-legislature.test.ts`.
  (b) **AbortController/wall-time-budget cancellation removed** — `apps/mcp-server/src/cache/refresh.ts` deletes `WallTimeBudgetExceeded`, the shared `AbortController`, `setTimeout(() => controller.abort(), wallTimeLimitMs)`, the per-session `controller.signal.aborted` pre-check, and the per-batch abort-cause reclassification; `apps/mcp-server/src/providers/types.ts` removes `signal?: AbortSignal` from `getBillDetail`'s contract; `apps/mcp-server/src/providers/utah-legislature.ts` removes the `signal` parameter, `isAbortError` helper, and abort-aware retry predicate.
  (c) **Log level changes** — `refresh.ts`: `logger.warn → logger.error` for "Failed to fetch bill stubs", `logger.debug → logger.error` for "getBillDetail failed — skipping", and the `logger.info({...},'all bills fresh — skipping')` line removed.
  Options: (1) Revert all three, ship clean 4.10, file separate stories. (2) Keep them, update spec File List + add new ACs covering them. (3) Cherry-pick: keep some, revert others.

**Patches (unchecked):**

- [ ] [Review][Patch] **CRITICAL — `warmUpBillsCache` does not propagate `fullText` to `writeBills`** [`apps/mcp-server/src/cache/refresh.ts:152-162`] — The provider `getBillsBySession` was updated per AC 9, but the production cron path (`warmUpBillsCache` → `getBillDetail` → manual `BillDetail`→`Bill` mapping) maps fields by hand and omits `fullText`. Every cron refresh writes `fullText: undefined`, which `writeBills` stores as NULL — clobbering any previously-cached value. Story 4.10's core deliverable is broken on the production data path. Add `...(detail.fullText !== undefined && { fullText: detail.fullText }),` after the `voteDate` spread.
- [ ] [Review][Patch] **Empty-string `fullText` violates absence semantics** [`apps/mcp-server/src/cache/bills.ts:317`] — `bill.fullText !== undefined ? stripHtml(bill.fullText) : null` writes empty string when `fullText === ''`. `rowToBill` then sees `row.full_text !== null` and assigns `bill.fullText = ''` rather than leaving it undefined. AC 11's "property absent" contract breaks if API returns empty `highlightedProvisions`. Change to `bill.fullText !== undefined && bill.fullText !== '' ? stripHtml(bill.fullText) : null` (or normalize whitespace-only output of `stripHtml` to null).
- [ ] [Review][Patch] **Story File List is incomplete** [`_bmad-output/implementation-artifacts/4-10-highlighted-provisions-full-text.md` File List section] — File List declares 10 paths; diff modifies 23. Undeclared: `CLAUDE.md`, `apps/mcp-server/eslint.config.cjs`, `apps/mcp-server/package.json`, `apps/mcp-server/src/lib/logger.ts`, `apps/mcp-server/src/lib/logger.test.ts`, `apps/mcp-server/src/lib/gis.test.ts`, `apps/mcp-server/src/cache/refresh.ts`, `apps/mcp-server/src/cache/refresh.test.ts`, `apps/mcp-server/src/providers/types.ts`, `apps/mcp-server/src/providers/utah-legislature.ts` (only the spread line is in scope; the abort-removal and logger-import reorder are not), `apps/mcp-server/src/tools/legislator-lookup.test.ts`, `apps/mcp-server/src/tools/search-bills.test.ts`, `apps/mcp-server/src/worker.ts`, `pnpm-lock.yaml`. Update File List once decision-needed is resolved.
...
- [ ] [Review][Patch] **FTS5-only-on-fullText test does not actually prove `full_text` was the matching column** [`apps/mcp-server/src/cache/bills.test.ts:531-541`] — Test writes one bill with `fullText: 'uniqueprovisionterm…'` and asserts the search returns it. Passes whether the match came from `full_text` or any other indexed column. Add a second control bill (same title/summary, no `fullText`) and assert it does NOT appear in results.
- [ ] [Review][Patch] **`logger.ts` imports `pino` but only uses the type** [`apps/mcp-server/src/lib/logger.ts:1`] — `import pino from 'pino'` pulls pino + sonic-boom + thread-stream + atomic-sleep into the Workers bundle (visible in `pnpm-lock.yaml`), but the runtime never calls `pino()`. `cfLogger` is hand-rolled and cast `as unknown as pino.Logger`. Change to `import type { Logger } from 'pino'` and use `Logger` directly. (Contingent on decision-needed (a); if logger refactor is reverted, this is moot.)
- [ ] [Review][Patch] **`logger.ts` violates the new `no-console` rule it introduces, via `eslint-disable-line`** [`apps/mcp-server/src/lib/logger.ts:79-81`] — `eslint.config.cjs` adds `'no-console': ['error', { allow: ['error'] }]` and `CLAUDE.md` declares "console.log FORBIDDEN", but `cfWrite` calls `console.warn(serialized) // eslint-disable-line no-console` and `console.log(serialized) // eslint-disable-line no-console`. The disable comments admit the new rule is the wrong shape: it should be `'no-console': ['error', { allow: ['error', 'warn', 'log'] }]` *only inside this file*, or the rule itself should be reframed. Either fix the file-scoped exception in eslint config or rephrase the CLAUDE.md rule. (Contingent on decision-needed (a).)
- [ ] [Review][Patch] **`logger.ts` Proxy + `as unknown as pino.Logger` cast hides API mismatch** [`apps/mcp-server/src/lib/logger.ts:31-51`] — `cfLogger` only implements trace/debug/info/warn/error/fatal. Any caller using pino-specific surface (`logger.child(...)`, `logger.level`, `logger.bindings()`, `logger.isLevelEnabled(...)`) will get `undefined` and throw at runtime — TypeScript can't catch it because of the `as unknown as` cast. Also documented in the diff itself: `legislator-lookup.test.ts` adds the comment "Required because logger is a Proxy — vi.spyOn fails on Proxies." Either implement the full interface or drop the cast and export a narrow shape (e.g. `Pick<Logger,'trace'|'debug'|'info'|'warn'|'error'|'fatal'>`). (Contingent on decision-needed (a).)
- [ ] [Review][Patch] **`getBillDetail` lost its caller-error logging** [`apps/mcp-server/src/providers/utah-legislature.ts` retry path] — Previous version logged the upstream HTTP status / non-JSON body before throwing; the retry wrapper then surfaces only `'getBillDetail failed after retries'`. Operators can no longer distinguish "API returns 502 for bill X" from "API returns HTML maintenance page". Restore status/short-body logging at the inner failure points before throwing. (Contingent on decision-needed (b).)
- [ ] [Review][Patch] **Wall-time budget cannot cancel in-flight `Promise.allSettled` batches** [`apps/mcp-server/src/cache/refresh.ts:122-148`] — Budget check is at the *start* of each batch iteration. With AbortController removed, a single batch of 20 `getBillDetail` calls with retries can run 4× longer than the wall-time budget; CF Workers may kill the worker mid-`writeBills`, losing the partial fetch. Restore an AbortController (or wrap each `getBillDetail` in `Promise.race` with a per-deadline timer). (Contingent on decision-needed (b); only meaningful if the abort path is restored.)
- [ ] [Review][Patch] **`writeBills` rebuild has no failure handling** [`apps/mcp-server/src/cache/bills.ts` rebuild line, post-batch insert] — `INSERT INTO bill_fts(bill_fts) VALUES('rebuild')` is unguarded; if it rejects (D1 timeout, transient error), the bills table is updated but the FTS5 index is stale. With the new third column, rebuild time grows. Wrap in try/catch and log; consider an on-demand "FTS dirty" flag that the next refresh repairs.
...
- [x] [Review][Defer] `wallTimeSeconds` configured to 1 or 2 silently no-ops the entire refresh [`apps/mcp-server/src/cache/refresh.ts:74-78`] — pre-existing arithmetic (`(wallTimeSeconds - 2) * 1000`); deferred.
- [x] [Review][Defer] Removing `signal?: AbortSignal` from `LegislatureDataProvider.getBillDetail` is a non-additive interface change [`apps/mcp-server/src/providers/types.ts:17`] — only out-of-tree consumers affected; no known consumers; deferred.
...
- [x] [Review][Defer] `searchBills` FTS5 path lacks try/catch around `prepare().first()`/`.all()` — invalid FTS5 syntax in user query becomes a hard 'temporarily unavailable' instead of empty results [`apps/mcp-server/src/cache/bills.ts:238-241`] — pre-existing; `searchBillsByTheme` has the guard; this path does not. Mirror that guard. Deferred, pre-existing.

**Dismissed as noise (5):** import-ordering churn in `refresh.ts`/`utah-legislature.ts`/`worker.ts`; whitespace edit in `gis.test.ts`; multi-line reformat of `expect.objectContaining` in `search-bills.test.ts`; `logger.test.ts` shape-of-output assertion staleness (subsumed by scope decision); test-mock comment churn in `legislator-lookup.test.ts`/`utah-legislature.test.ts` (subsumed by scope decision).
