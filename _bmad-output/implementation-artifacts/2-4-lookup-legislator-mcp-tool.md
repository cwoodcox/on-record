...
| 2026-03-04 | Retrospective: Live testing revealed four bugs in `legislator-lookup.ts` (duplicate GIS logic, never exercised `gis.ts`): (1) `parseAddress` split on last comma → zone `"UT 84096"` rejected by UGRC (expects city or ZIP only); (2) geometry format `point:${x},${y}` should be ArcGIS JSON `point:{"x":${x},"y":${y}}`; (3) table names `political.state_house_districts`/`political.state_senate_districts` should be `political.house_districts_2022_to_2032`/`political.senate_districts_2022_to_2032`; (4) field name `DIST` should be `dist`. Fix: removed `parseAddress`, changed tool schema to separate `street` + `zone` inputs (LLM handles address parsing), corrected geometry/tables/field. Commit `27ab494`. Root cause: no real API calls during dev or review — all tests used mocks. | Corey |
